...
e.message += '\n请报告这 https://github.com/chjj/marked.';
if ((opt || marked.defaults).silent) {
return '<p>发生了一个错误:</p><pre>' + escape(e.message + '', true) + '</pre>';
}
throw e;
}
}
...
Polymer.Base._error(this, 'received both a bound descriptor:', this.descriptor, 'and JSON descriptor:', this._jsonDescriptor, '请只提供一个');
throw new Error('<iron-doc-viewer> 接受任何束缚或JSON描述符; not both');
}
if (jsonDescriptor) {
...
this._privateToggleLabel = (this._showPrivate ? '隐藏' : '显示') + ' 自定 API';
...
this.router.go('../' + e.currentTarget.getAttribute('name'));
},
navToBehavior: function (e, detail) {
if (this.behaviorMap[detail]) {
this.router.go('../' + this.behaviorMap[detail] + '?active=' + detail);
} else {
this.fire('toast', { text: '没有可用的文档 ' + detail });
}
},
...
return window.location.origin + '../' + element + '/' + element + '.html';
},
search: function (q) {
if (q || this.q) {
this.router.go('../browse?q=' + (q || this.q));
